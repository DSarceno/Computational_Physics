...
!    gfortran -Wall -pedantic -std=f95 -c -o randomSphereGOOD.o randomSphereGOOD.f90
!    gfortran -Wall -pedantic -std=f95 -o randomSphereGOOD.x types.o randomSphereGOOD.o
...
PROGRAM randomSphereGOOD
  ...
  ! saludar
  WRITE (*,*) "# Coordenadas cartesianas obtenidas de la generación"
  ...
  DO i=1,runs
     ...
     theta=2*PI*theta
     phi=ACOS(2.*phi-1.)
     WRITE (*,*) SIN(phi)*COS(theta), SIN(phi)*SIN(theta), COS(phi)
  END DO

END PROGRAM randomSphereGOOD
