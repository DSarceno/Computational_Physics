...
!    2012.  En esta versión se envían los subtotales para que el rank 0
!    calcule el total
...
PROGRAM trapecioP
  ...
  IF (rank.EQ.0) THEN
     Total=Area
     DO emisor=1,size-1,1
        CALL MPI_RECV(Area, 1, MPI_DOUBLE, emisor, tag, MPI_COMM_WORLD, status, err)
        Total=Total+Area
     END DO
     WRITE (*,*) "El area total es",Total
  ELSE
     CALL MPI_SEND(Area, 1, MPI_DOUBLE, 0, tag, MPI_COMM_WORLD, err)
  END IF
...
!MPI_SEND(data, count, MPI_TYPE, to, tag, MPI_COMM_WORLD, ierr )
!MPI_RECV(data, count, MPI_TYPE, from, tag, MPI_COMM_WORLD, status, ierr ) 
